...
function viteLegacyPlugin(options = {}) {
  ...
  const legacyGenerateBundlePlugin = {
    ...
    async generateBundle(opts, bundle) {
      ...
      if (legacyPolyfills.size || genDynamicFallback) {
        ...
        const { code } = await buildPolyfillChunk(
          'polyfills-legacy',
          legacyPolyfills,
          bundle,
        ...
        legacyContent.set('polyfills-legacy', code)
      }
    },
  };
  ...
  const legacyPostPlugin = {
    ...
    configResolved(_config) {
       ...
       * @param {string} [defaultFileName="[name]-legacy.[hash].js"]
      ...
      if (Array.isArray(output)) {
        rollupOptions.output = [...output.map(createLegacyOutput)];
      } else {
    ...
    renderChunk(raw, chunk, opts) {
      ...
      legacyContent.set(chunk.fileName, code);
    ...
    transformIndexHtml(html, { chunk }) {
      ...
      console.log(legacyPolyfillFilename, legacyContent.keys())
      if (legacyPolyfillFilename) {
        tags.push({
          ...
          injectTo: 'body',
          children: `//legacy polyfills\n${legacyContent.get('polyfills-legacy')}`
        });
      ...
      if (legacyEntryFilename) {
        tags.push({
          ...
          children:
            ...
            legacyEntryFilename +
            '\n' +
            legacyContent.get(legacyEntryFilename),
          injectTo: 'body',
        });
      } else {
        throw new Error(
    ...
    generateBundle(opts, bundle) {
      ...
      //     bundle[name].fileName.endsWith('.js') &&
      //     !isLegacyChunk(bundle[name], config.build.rollupOptions.output)
      //   ) {
      //     delete bundle[name];
      //   }
      // }
 ...
 * @returns {Promise<import('rollup').OutputChunk>}
 */
