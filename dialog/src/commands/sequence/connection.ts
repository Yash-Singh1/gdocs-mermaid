export const match =
  /^(.*?)\s+(is\s+)?(connect|dot|broke|break)(n|s|t?ed|t?ing)?(\s*(tw?oo?|2))?\s+(.*?)\s*(fou?r|4)\s*(.*?)$/;

export function manipulate(answer: RegExpExecArray): string {
  return `${answer[1]}${answer[3]}${answer[7]}: ${answer[answer.length - 1]}`;
}
...
export function cleanMatch(match: RegExpExecArray) {
  ...
  match[7] = cleanMatchPart(match[7]);
  if (match[3] === 'connect') {
    match[3] = '->>';
  } else if (match[3] === 'dot') {
    match[3] = '-->>';
